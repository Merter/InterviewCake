...
public class Solution {
    ...
    public static void reverseWords(char[] message) {
        ...
        final ArrayDeque<Integer> delimiterPositionQueue = new ArrayDeque<>();
        for (int i=0; i<message.length; i++) {
            if (message[i] == ' ') {
                delimiterPositionQueue.addLast(i);
            }
        }
        if (delimiterPositionQueue.isEmpty()) {
            return;
        }
        
        int begin = 0;
        int end = message.length-1;
        while (!delimiterPositionQueue.isEmpty()) {
            end = delimiterPositionQueue.removeFirst()-1;
            reverseInBetween(message, begin, end);
            begin = end+2;
        }
        reverseInBetween(message, begin, message.length-1);
        reverseInBetween(message, 0, message.length-1);
    }

    private static void reverseInBetween(char[] message, int from, int to) {
        for (int i=from; i<=(from+to)/2; i++) {
            char temp = message[i];
            message[i] = message[to-i+from];
            message[to-i+from] = temp;
        }
    }
